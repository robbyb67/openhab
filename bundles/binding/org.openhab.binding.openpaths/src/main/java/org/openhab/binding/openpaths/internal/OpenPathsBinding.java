 ...
 * @since 1.4.0
 */
public class OpenPathsBinding extends
		AbstractActiveBinding<OpenPathsBindingProvider> implements
		ManagedService {

	private static final Logger logger = LoggerFactory
			.getLogger(OpenPathsBinding.class);
...
>>>>>>> Stashed changes
	...
	public void execute() {
		...
		for (OpenPathsBindingProvider provider : providers) {
			for (String itemName : provider.getItemNames()) {
				...
				OpenPathsBindingConfig bindingConfig = provider
				...
				if (!openPathsUsers.containsKey(name)) {
					logger.warn("There is no OpenPaths user configured for '"
							+ name
				...
				if (openPathsUser.lastUpdateTS + this.refreshInterval < System
						.currentTimeMillis()) {
					String accessKey = openPathsUser.getAccessKey();
					String secretKey = openPathsUser.getSecretKey();

					if (StringUtils.isEmpty(accessKey)) {
						logger.warn("There is no ACCESS_KEY configured for '"
								+ name
								+ "'. Please add this user to the binding configuration, including both the ACCESS_KEY and SECRET_KEY from the OpenPaths profile.");
						continue;
					}
					if (StringUtils.isEmpty(secretKey)) {
						logger.warn("There is no SECRET_KEY configured for '"
								+ name
								+ "'. Please add this user to the binding configuration, including both the ACCESS_KEY and SECRET_KEY from the OpenPaths profile.");
						continue;
					}
					...
					if (location != null) {
						...
						logger.debug("New location received for '{}': {}",
								name, location.toString());
					} else {
						logger.warn(
								"Unable to determine location for '{}'. Skipping.",
								name);
						continue;
					}
				} else {
					location = openPathsUsers.get(name).getLastLocation();
					logger.trace("Using cached location for '{}'",
							openPathsUser.toString());
				}
				...
				if (bindingLocationName.startsWith("current")) {
					if (bindingLocationName.equals("currentLocation")) {
						eventPublisher.postUpdate(itemName,
								new StringType("" + location.getLatitude()
						...
						logger.warn("unsupported Binding: "
								+ bindingLocationName);
					}
					continue;
				}
				...
				if (!locations.containsKey(bindingLocationName)) {
					logger.warn("location name " + bindingLocationName
							+ " not configured, falling back to 'home'");
					bindingLocationName = "home";
				}
				logger.debug("OpenPathsUser: " + name + "@"
						+ bindingLocationName);
				...
				logger.trace(
						"Calculating distance between home ({}) and user location ({}) for '{}'...",
						new Object[] { bindingLocation.toString(),
								location.toString(), name });
				double distance = calculateDistance(bindingLocation, location);
				bindingLocation.setDistance(distance);
				logger.trace("Distance calculated as {} for '{}'@'{}'",
						distance, name, bindingLocationName);

				if (bindingType.equals(LocationBindingType.on)) {
					float fence = bindingLocation.getGeofence() == 0.0 ? geoFence
							: bindingLocation.getGeofence();
					if (distance <= fence) {
						logger.trace(
								"Detected that '{}'@'{}' is inside the geofence ({}m)",
								name, bindingLocationName, fence);
						eventPublisher.postUpdate(itemName, OnOffType.ON);
					} else {
						logger.trace(
								"Detected that '{}'@'{}' is outside the geofence ({}m)",
								name, bindingLocationName, fence);
						eventPublisher.postUpdate(itemName, OnOffType.OFF);
					}
				} else if (bindingType.equals(LocationBindingType.distance)) {
					eventPublisher.postUpdate(itemName, new DecimalType(
							new BigDecimal(distance / 1000)));
				}
			}
		}
	}
	...
	private Location getUserLocation(String accessKey, String secretKey) {
		...
		OAuthService service = new ServiceBuilder()
				.provider(new OpenPathsApi()).apiKey(accessKey)
		...
		if (!response.isSuccessful()) {
			logger.error("Failed to request the OpenPaths location, response code: "
					+ response.getCode());
			return null;
		}
	...
	public void updated(Dictionary<String, ?> config)
		...
		if (config != null) {
				...
				if ("refresh".equals(key)) {
					...
					if (keyParts.length != 2) {
						throw new ConfigurationException(key,
								"Invalid OpenPaths user location lattitude: "
										+ key);
					}
					if (StringUtils.isNotBlank(value)) {
						...
						if (locations.containsKey(name)) {
							...
							logger.trace("Config: new Location " + name + "("
									+ locations.get(name) + ")");
						} else {
							Location loc = new Location();
							loc.setLatitude(lat);
							logger.trace("Config: update Location " + name
									+ "(" + locations.get(name) + ")");
					...
					if (keyParts.length != 2) {
						throw new ConfigurationException(key,
								"Invalid OpenPaths user location longitude: "
										+ key);
					}
					if (StringUtils.isNotBlank(value)) {
					...
					if (keyParts.length != 2) {
						throw new ConfigurationException(key,
								"Invalid OpenPaths user location geofence: "
										+ key);
					}
					if (StringUtils.isNotBlank(value)) {
					...
					if (keyParts.length != 2) {
						throw new ConfigurationException(key,
								"Invalid OpenPaths user key: " + key);
					}
			...
			if (!locations.containsKey("home"))
				throw new ConfigurationException("home.lat",
						"No location specified for 'home'");
